...
from models.memorybank import HybridMemory
from data.cifarloader import CIFAR10Loader, CIFAR10LoaderMix, CIFAR100Loader, CIFAR100LoaderMix, CIFAR10Loader_iter
...
def train(model, model_ema, memorybank, labeled_eval_loader_train, unlabeled_eval_loader_test, unlabeled_eval_loader_train, args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 5 == 0:
            args.head = 'head2'
            feats, feats_mb,_ = test(model_ema, unlabeled_eval_loader_train, args)
            ...
            memorybank.labels = torch.Tensor(labels_.reshape(-1).tolist()).long().cuda()
        ...
        for batch_idx, (((x_l, x_bar_l), label_l, idx_l), ((x_u, x_bar_u), label_u, idx_u)) in enumerate(zip(labeled_train_loader, unlabeled_train_loader)):
            ...
            loss_in_unlabel = torch.tensor(0)#memorybank(feat_mb[~mask_lb], feat_mb_ema[~mask_lb], label[~mask_lb], idx[~mask_lb])
            ...
            if batch_idx % 200 == 0:
                print(
                    ...
                    .format(epoch, batch_idx, 400, loss_ce_unlabel.item(), loss_in_unlabel.item(),loss_ce_label.item(),  loss_bce.item(),
                            consistency_loss.item(), loss_record.avg))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--warmup_model_dir', type=str,
                        default='./data/experiments/pretrained/supervised_learning/resnet_rotnet_cifar10.pth')
    ...
    memorybank = HybridMemory(128, len(unlabeled_eval_loader_train.dataset.targets)).to(device)
    # memorybank = HybridMemory(512, args.num_unlabeled_classes + len(unlabeled_eval_loader_train.dataset.targets)).to(device)
    ...
    if args.mode == 'train':
        train(model, model_ema, memorybank, labeled_eval_loader_train, unlabeled_eval_loader_test, unlabeled_eval_loader_train,args)
