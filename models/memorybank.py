...
class HM(autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        grad_inputs = None
        if ctx.needs_input_grad[0]: grad_inputs = grad_outputs.mm(ctx.features)
...
class HybridMemory(nn.Module):
    ...
    def forward(self, inputs, inputs_ema, targets, indexes):
        ...
        sim = torch.zeros(labels.max()+1, B).float().cuda()#5,64
        sim.index_add_(0, labels, inputs.t().contiguous())
        nums = torch.zeros(labels.max()+1, 1).float().cuda()#5,1
